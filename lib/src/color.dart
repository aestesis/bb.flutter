  ...
  String toCssString({CssColorString format = CssColorString.hex}) {
    switch (format) {
      ...
      case CssColorString.rgb:
        var r = (this.r * 255).round();
        var g = (this.g * 255).round();
        var b = (this.b * 255).round();
        var a = (this.a * 255).round();
...
enum ColorFormat { hex, rgb, rgba, hsl, hsla, keyword }
...
Color _hexToColor(String color) {
  ...
  return Color(
    0x1000000 * int.parse(alpha, radix: 16) + int.parse(color, radix: 16),
  );
}
...
List<String>? _parseChannels(String color) {
